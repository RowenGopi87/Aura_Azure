...
const MODULE_NAMES = {
  'design': 'Design Reverse Engineering',
  'code': 'Code Reverse Engineering',
};

interface ReverseEngineeringSettingsProps {
  showActions?: boolean;
}

export function ReverseEngineeringSettings({ showActions = false }: ReverseEngineeringSettingsProps) {
  const {
    reverseEngineeringLLMSettings,
    availableProviders,
    setReverseEngineeringLLM,
  ...
  const handleModuleLLMChange = (
    ...
    setReverseEngineeringLLM(module, provider, model);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error saving reverse engineering settings:', error);
      setSaveStatus('error');
    } finally {
      setIsSaving(false);
    }
  ...
  const validateModuleSettings = (module: 'design' | 'code') => {
    const config = reverseEngineeringLLMSettings[module];
    return !!(config.provider && config.model && hasApiKey);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {showActions && (
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          ...
          <p className="text-sm text-purple-800 mb-2">
            Configure which LLM to use for Design and Code reverse engineering operations.
          </p>
          {!hasApiKey && (
      ...
      <div className="space-y-3">
        {(['design', 'code'] as const).map((module) => {
          const Icon = MODULE_ICONS[module];
          const isValid = validateModuleSettings(module);
          const config = reverseEngineeringLLMSettings[module];
              ...
              <CardContent className="pt-0">
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-gray-700">LLM Provider & Model</label>
                    <Select
                      value={`${config.provider}:${config.model}`}
                      onValueChange={(value) => handleModuleLLMChange(module, value)}
                    >
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-blue-600">
              {(['design', 'code'] as const).filter(validateModuleSettings).length}/2
            </span>
          </div>
        </div>
      </div>
